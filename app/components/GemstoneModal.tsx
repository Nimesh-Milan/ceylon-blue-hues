...
export default function GemstoneModal({ gemstone, onClose }: GemstoneModalProps) {
    ...
    const activeMedia = gemstone?.media.find((m) => m.file_path === activeMediaPath) ?? gemstone?.media[0] ?? null;

    useEffect(() => {
        if (gemstone) {
            setIsShowing(true);
            setActiveMediaPath(gemstone.media[0]?.file_path ?? null);
                                ...
                                {gemstone.media.map((mediaItem) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            onClick={() => setActiveMediaPath(mediaItem.file_path)}

        ...
        <div className="min-h-screen bg-cream py-10 px-6">
            <div className="max-w-4xl mx-auto">
                ...
                <div id="inquiries" className="mt-16">
                    <h2 className="text-2xl font-light text-stone mb-6">Inquiries</h2>
                    {loading ? (
                        <div className="bg-white border border-mid/15 rounded-xl overflow-hidden">
                            <SkeletonRow />
                        </div>
                        ...
                        <div className="bg-white border border-mid/15 rounded-xl overflow-hidden">
                            {inquiries.map((inquiry) => (
                                <div key={inquiry.id} className="px-6 py-4 border-b border-mid/10 last:border-0">
                                    <div className="flex items-baseline justify-between">
                                        <div>
                                            <p className="text-stone font-medium">{inquiry.name}</p>
                                            <a href={`mailto:${inquiry.email}`} className="text-sm text-gold hover:text-stone transition-colors">{inquiry.email}</a>
                                        </div>
                                        <p className="text-xs text-mid">{new Date(inquiry.created_at).toLocaleDateString()}</p>
                                    </div>
                                    <p className="text-sm text-mid mt-2">{inquiry.message}</p>
                                    {inquiry.gemstone_slug && (
                                        <Link href={`/gemstones/${inquiry.gemstone_slug}`} className="text-xs text-gold hover:text-stone transition-colors mt-2 inline-block">
                                            Inquired about: {inquiry.gemstone_slug}
                                        </Link>
                                    )}
                                </div>
                            ))}

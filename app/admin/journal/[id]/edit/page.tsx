...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (loading) return <div className="flex justify-center py-32"><div className="w-8 h-8 border-t-2 border-gold rounded-full animate-spin" /></div>;

    return (
        <div className="w-full max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 border-b border-navy/5 pb-12">
                <div>
                    <h1 className="text-[6vw] md:text-4xl lg:text-5xl font-serif italic text-navy mb-4 leading-none tracking-tight">Edit Story</h1>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-12">
                ...
                <div className="flex flex-col gap-8">
                    <div>
                        ...
                        <input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-transparent border-b border-navy/20 py-4 text-3xl md:text-5xl font-serif italic text-navy focus:outline-none focus:border-gold transition-colors placeholder-navy/20" />
                    </div>
                ...
                <div className="flex flex-col gap-8">
                    <div>
                        ...
                        <textarea value={form.excerpt} onChange={e => setForm({...form, excerpt: e.target.value})} className="w-full bg-transparent border-b border-navy/20 py-3 text-xl font-serif italic text-navy focus:outline-none focus:border-gold transition-colors placeholder-navy/20 h-24 resize-none" />
                    </div>
                    <div>
                        <label className="block text-[9px] uppercase tracking-[0.3em] font-bold text-navy/40 mb-4">Content (HTML)</label>
                        <textarea required value={form.content} onChange={e => setForm({...form, content: e.target.value})} className="w-full bg-transparent border-b border-navy/20 py-3 text-lg font-sans text-navy focus:outline-none focus:border-gold transition-colors placeholder-navy/20 min-h-[300px] resize-y" />
                    </div>
                </div>
